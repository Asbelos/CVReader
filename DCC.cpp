...
void DCC::writeCVBit(int cv, byte bitNum, bool bitValue, ACK_CALLBACK callback, bool blocking)
{
  if (bitNum >= 8)
    ...
    ackManagerSetup(cv, bitNum, bitValue ? WRITE_BIT1_PROG : WRITE_BIT0_PROG, callback, blocking);
}
...
void DCC::verifyCVBit(int cv, byte bitNum, bool bitValue, ACK_CALLBACK callback, bool blocking)
{
  if (bitNum >= 8)
    ...
    ackManagerSetup(cv, bitNum, bitValue ? VERIFY_BIT1_PROG : VERIFY_BIT0_PROG, callback, blocking);
}
...
void DCC::readCVBit(int cv, byte bitNum, ACK_CALLBACK callback, bool blocking)
{
  if (bitNum >= 8)
    ...
    ackManagerSetup(cv, bitNum, READ_BIT_PROG, callback, blocking);
}
...
void DCC::issueReminders()
{
  ...
  if (DCCWaveform::mainTrack.packetPending)
    return;
...
void DCC::ackManagerLoop(bool blocking)
{
  while (ackManagerProg)
  {
    ...
    if (Diag::ACK)
          DIAG(F("\nackManagerLoop:ackManagerProg:opcode [%d] blocking mode [%d]"), opcode, blocking);
...
///////////////
      ...
      if (checkResets(blocking, DCCWaveform::progTrack.autoPowerOff ? 20 : 3))
        return;
    ...
    case W1: // write 1 bit
    {
      if (checkResets(blocking, RESET_MIN))
        return;
      if (Diag::ACK)
    ...
    case WACK: // wait for ack (or absence of ack)
    {
      // grbba
      if (Diag::ACK)
        DIAG(F("\nWACK begin blocking mode [%d]"), blocking);
      ...
      if (blocking)
      {
        // grbba
        if (Diag::ACK)
             DIAG(F("\nWACK ackState in blocking before while : [%d]"), ackState);
        while (ackState == 2) {
          ackState = DCCWaveform::progTrack.getAck();
          // grbba
          // if (Diag::ACK)
          //   DIAG(F("\nWACK ackState in blocking -in- while : [%d]"), ackState);
        }
          // grbba
          if (Diag::ACK)
             DIAG(F("\nWACK ackState in blocking after while : [%d]"), ackState);
